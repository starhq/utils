...
public class IdcardUtil {
    ...
    public static String convert15To18(String idCard) {
        ...
        if (NumberUtil.isNumber(idCard)) {
            ...
            LocalDate localDate = LocalDate.parse(idCard.substring(6, 12), DateTimeFormatter.ofPattern("yyMMdd"));
            // 获取出生年(完全表现形式,如：2010)
            int sYear = localDate.getYear();
            if (sYear > 2000) {
                //2000年之后不存在15位身份证号，此处用于修复此问题的判断
                sYear -= 100;
            }
            idCard18 = StringUtil.builder().append(idCard.substring(0, 6)).append(sYear).append(idCard.substring(8));
            // 获取校验位
    ...
    public static boolean isvalidCard15(String idCard) {
        ...
        if (NumberUtil.isNumber(idCard)) {
            ...
            // 生日
            LocalDate localDate = LocalDate.parse(idCard.substring(6, 12), DateTimeFormatter.ofPattern("yyMMdd"));
            return Validator.isBirthday(localDate.getYear(), localDate.getMonthValue(), localDate.getDayOfMonth());
        } else {
            return false;
        }
    }
    ...
    public static int getAgeByIdCard(String idCard, LocalDate dateToCompare) {
        ...
        LocalDate localDate = LocalDate.parse(birth, DateTimeFormatter.ofPattern("yyyyMMdd"));
        return (int) localDate.until(dateToCompare, ChronoUnit.YEARS);
    }
