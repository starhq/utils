...
public class ImageUtil {
    ...
    public static Image scale(Image srcImg, float scale) {
        ...
        // 得到源图宽
        int width = ArithmeticUtil.multiply(0, String.valueOf(srcImg.getWidth(null)), String.valueOf(scale)).intValue();
        // 得到源图长
        int height = ArithmeticUtil.multiply(0, String.valueOf(srcImg.getHeight(null)), String.valueOf(scale)).intValue();
    ...
    public static Image scale(Image srcImage, int width, int height, Color fixedColor) {
        ...
        double heightRatio = ArithmeticUtil.divide(String.valueOf(height), String.valueOf(srcHeight), 0).doubleValue();
        double widthRatio = ArithmeticUtil.divide(String.valueOf(width), String.valueOf(srcWidth), 0).doubleValue();
    ...
    public static void sliceByRowsAndCols(Image srcImage, File destDir, int rows, int cols) {
        if (!destDir.exists()) {
            PathUtil.mkParentDirs(destDir.toPath());
     ...
     * @throws IORuntimeException IO异常
     */
    public static void pressText(Image srcImage, ImageOutputStream destImageStream, String pressText, Color color, Font font, int x, int y, float alpha) throws
            IORuntimeException {
    ...
    public static void write(Image image, File targetFile) throws IORuntimeException {
        String formatName = PathUtil.extName(targetFile);
        if (StringUtil.isBlank(formatName)) {
            formatName = IMAGE_TYPE_JPG;
        }
        try {
            // 输出到文件
            ImageIO.write(toBufferedImage(image), formatName, targetFile);
        } catch (IOException e) {
            throw new IORuntimeException(e);
        }
    }
