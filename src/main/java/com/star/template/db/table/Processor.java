...
import com.star.exception.TableException;
...
import com.star.template.db.util.ColumnUtil;
import com.star.template.db.util.DatabaseUtil;
import com.star.template.db.util.DatabaseVersionUtil;
import com.star.template.db.util.PropertyUtil;
import com.star.template.db.util.TypeUtil;
...
public class Processor {
    ...
    private String getOracleTableComments(final String tableName) {
        ...
        return sqlRunner.query(DatabaseUtil.getInstance().getConnection(), sql, resultSet -> {
            try {
    ...
    private List<Column> getTableColumn(final String tableName, final List<String> primaryKeys, final List<String>
        ...
        try (ResultSet columnRs = metaData.getColumns(PropertyUtil.getSchema(), PropertyUtil.getCatalog(), tableName,
            ...
            while (columnRs.next()) {
                ...
                final StringTokenizer stringTokenizer = new StringTokenizer(columnAlias, "\t" + StringUtil
    ...
    private String removePrefix(final String tableName) {
        ...
        for (final String prefix : prefixArray) {
            name = StringUtil.removePrefix(tableName, prefix, true);
        }
        return name;
    }
