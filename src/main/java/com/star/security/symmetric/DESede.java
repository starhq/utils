...
import com.star.security.Mode;
import com.star.security.Padding;
...
public class DESede extends SymmetricCrypto {
    ...
    public DESede(String mode, String padding, byte[] key, byte[] iv) {
        this(mode, padding, SecureUtil.generateKey(ALGORITHM.DESede.toString(), key), null == iv ? null : new IvParameterSpec(iv));
    }
    ...
    public DESede(String mode, String padding, SecretKey key, IvParameterSpec iv) {
        super(StringUtil.format("{}/{}/{}", ALGORITHM.DESede, mode, padding), key, iv);
    }
    // ------------------------------------------------------------------------- Constructor end
